...
public abstract class AbstractJsoupCookieDocumentSpider implements DocumentSpider, SpiderConstant {
    ...
    protected Document parseDocument(Connection.Response response) {
        try {
            ...
            logger.info("parseDocument fail", e);
            return null;
        }
    }
    ...
    protected Connection.Response executeConnection(Connection connection) {
        for (int i = 1; i <= RETRY_TIMES; i++) {
            try {
                ...
                if (i < RETRY_TIMES) {
                    logger.debug(String.format("executeConnection retry in %s, url=%s", i, connection.request().url().toString()), e);
                } else {
                    logger.info(String.format("executeConnection retry fail, url=%s", connection.request().url().toString()), e);
                }
            }
        }
        return null;
