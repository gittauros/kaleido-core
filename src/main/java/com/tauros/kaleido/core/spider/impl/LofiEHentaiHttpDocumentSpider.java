    ...
    protected HttpResponse executeGet(HttpGet get) {
        ...
        for (int i = 1; i <= RETRY_TIMES; i++) {
            try {
                ...
                if (i < RETRY_TIMES) {
                    logger.debug(String.format("executeGet retry in %s, url=%s", i, get.getURI()), e);
                } else {
                    logger.info(String.format("executeGet retry fail, url=%s", get.getURI()), e);
                }
            }
        }
        return null;
    ...
    private String getBody(HttpResponse response) {
        ...
        for (int i = 1; i <= RETRY_TIMES; i++) {
            ...
            try {
                ...
                if (i < RETRY_TIMES) {
                    logger.debug(String.format("getBody retry in %s, response=%s", i, String.valueOf(response)), e);
                } else {
                    logger.info(String.format("getBody retry fail, response=%s", String.valueOf(response)), e);
                }
            } finally {
